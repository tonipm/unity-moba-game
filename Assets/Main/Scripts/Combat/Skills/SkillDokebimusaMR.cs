...
public class SkillDokebimusaMR : Skill
{
    private int damage;
    ...
    void Start()
    {
        this.prefabActionName = "SkillsActions/SkillDokebimusaMR";
        this.damage = -26;
        this.manaCost = 0;
        this.coolDownTime = 1;
    ...
    public override bool Execute(int damageChamp)
    {
        if (this.canExecute)
        {
            ...
            go.GetComponent<ActionDokebimusaMR>().SetSkill(this);
            go.GetComponent<Collider>().enabled = true;

            Unidad jugador = this.gameObject.GetComponent<Unidad>();
            Action action = go.GetComponent<ActionDokebimusaMR>();
